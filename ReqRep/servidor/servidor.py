...
while True:
    ...
    if comando ==  "ADD":
        if len(partes) < 2:
            resposta = "Erro: informe tarefa"
        else:
            tarefas.append(partes[1])
            resposta = "Tarefa adicionada: {partes[1]}"
        ...
        if not tarefas:
            ...
            resposta = "\n".join(
                f"{i+1}. {t}" for i, t in enumerate(tarefas)
            )
    elif comando == "REMOVE":
        if len(partes) < 2:
            resposta = "Erro: informe numero "
        else:
            try:
